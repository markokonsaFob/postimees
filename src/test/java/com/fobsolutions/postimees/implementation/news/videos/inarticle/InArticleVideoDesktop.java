 ...
 */
public class InArticleVideoDesktop implements VideoComponent {

    @FindAll({
            @FindBy(className = "articleContent"),
            @FindBy(className = "article__body--content")
    })
    ...
    public InArticleVideoDesktop(Device device) {
    ...
    public void play() {
        if (getPlayer() != null) {
            if (device.getDriver() instanceof IOSDriver) {
                BrowserActions.scrollToElement(device, getPlayer());
